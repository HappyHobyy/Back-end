...
import org.v1.dto.TextArticleRequest;
import org.v1.dto.TextArticleResponse;
...
public class TextArticleController {
    ...
    public HttpResponse<Object> getArticleLatest(
            @RequestBody TextArticleRequest.LatestRequest articleRequest
    ) {
        List<TextArticle> articleList = textArticleService.getRecentTextArticles(articleRequest.communityId());
    ...
    public HttpResponse<Object> getArticleSearch(
            @RequestBody TextArticleRequest.SearchRequest articleRequest
    ) {
        List<TextArticle> articleList = textArticleService.getRecentTextArticlesBySearch(articleRequest.toSearch());
        return HttpResponse.success(TextArticleResponse.of(articleList));
    }
    @DeleteMapping("")
    ...
    public HttpResponse<Object> deleteArticle(
            @RequestBody TextArticleRequest.DeleteRequest articleRequest
    ) {
        textArticleService.deleteTextArticle(articleRequest.articleId());
    ...
    public HttpResponse<Object> createArticle(
            @RequestPart TextArticleRequest.CreateRequest articleRequest,
            @RequestPart("files") List<MultipartFile> files,
            @Parameter(hidden = true) @Valid @RequestHeader Long userId
    ) {
        Long articleId = textArticleService.createTextArticle(articleRequest.toArticle(userId),articleRequest.toContent(files));
