...
public class TextArticleDetailController {
    ...
    public HttpResponse<Object> createLikeArticle(
            @RequestBody TextArticleLikeRequest textArticleLikeRequest,
            @Parameter(hidden = true) @Valid @RequestHeader Long userId
    ) {
        textArticleDetailService.likeArticle(textArticleLikeRequest.toLike(userId));
    ...
    public HttpResponse<Object> deleteArticleLike(
        ...
        textArticleDetailService.unlikeArticle(textArticleLikeRequest.toLike(userId));
        return HttpResponse.successOnly();
    }
    ...
    public HttpResponse<DefaultId> createArticleComment(
            @RequestBody TextArticleCommentRequest.Create articleCommentRequest,
            @Parameter(hidden = true) @Valid @RequestHeader Long userId
    ) {
        Long commentId = textArticleDetailService.commentOnArticle(articleCommentRequest.toComment(userId),articleCommentRequest.articleId());
    ...
    public HttpResponse<Object> deleteArticleComment(
            @RequestBody TextArticleCommentRequest.Delete articleCommentRequest
    ) {
        textArticleDetailService.deleteArticleComment(articleCommentRequest.commentId());
        return HttpResponse.successOnly();
    }
}
